    ...
    for episode in range(args.episode):
        ...
        for t in itertools.count(start=1):
            ...
            if done:
                ...
                break
    env.close()
    return episode_list, ewma_reward_list
...
def main():
    ...
    if not args.test_only:
        episode_list, ewma_reward_list = train(args, env, agent, writer)
        agent.save(args.model)

    agent.load(args.model)
    test(args, env, agent, writer)

    plt.plot(episode_list, ewma_reward_list)
    plt.ylabel('Ewma Reward')
    plt.xlabel('Episode')
    plt.savefig('Episode Reward - DDQN')
    plt.show()
