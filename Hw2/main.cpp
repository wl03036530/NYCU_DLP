...
public:
	 ...
	 *  value() is the estimated value of previous after_state()
	 *
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) 
			{
				...
				/* Dont know why it will get some error
				int space[16], space_num = 0;
				for (int i = 0; i < 16; i++)
					if (move->after_state().at(i) == 0)
						space[space_num++] = i;
				...
				if(space_num != 0)
				{
					for(int i = 0; i < space_num; i +=1)
					{
						// appear 2, 90%
						board new_board = move->after_state();
						new_board.set(space[i], 1);
						value += (1/space_num) * 0.9 * estimate(new_board);
						// appear 4, 10%
						new_board.set(space[i], 2);
						value += (1/space_num) * 0.1 * estimate(new_board);
					}
				}
				*/
...
int main(int argc, const char* argv[]) {
	...
	for (size_t n = 1; n <= total; n++) {
		...
		/*if(n % 100000 == 0)
		{
			...
			std::string s("traned_");
			s += std::to_string(n/10000);
			tdl.save(s);
		}*/
	}

	// store the model into file
	//tdl.save("traned_final");
